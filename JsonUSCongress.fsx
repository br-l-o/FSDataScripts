...
[<Literal>]
let senatorsJson = "https://www.govtrack.us/api/v2/role?current=true&role_type=senator"
...
let getSenatorsByState state =
    senators.Objects |> Array.filter (fun x -> x.State = state) |> Array.map (fun x -> {
        Body = x.RoleType
        Name = $"{x.Person.Firstname} {x.Person.Lastname}"
        State = x.State
        District = None
...
let printCongressMembersByState state =
    let members = (state |> getRepresentativesByState) |> Array.append (state |> getSenatorsByState)
    for i in members do
        printfn $"\nName: {i.Name}"
        printfn $"State: {i.State}"
        printfn $"Body: {i.Body}"
        match i.District with
        | Some x -> printfn "District: %s" (if x = 0 then "At-large" else x |> string)
        | _ -> ()
        
let numberOfMembersByState state =
        (state |> getRepresentativesByState) |> Array.append (state |> getSenatorsByState) |> Array.length

printfn "Representatives from the 1st district (or at-large representative) of their respective state/territory: "
for i in representatives.Objects
         |> Array.filter (fun x -> x.District <= 1)
         |> Array.sortBy (fun x -> x.State) do
    let districtType = if i.District < 1 then "At-large" else "1st"
...
for i in "NY" |> getRepresentativesByState |> Array.filter (fun x -> x.District.Value < 18) do
