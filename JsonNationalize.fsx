...
let countryCodesDict = 
    let codes = CountryCodes.Load("http://country.io/names.json");
    JsonValue.Parse(codes.JsonValue.ToString()).Properties()
...
let MostLikelyCountryOfOrigin name =
    ...
    | Some serializedNameDetails -> 
        ...
        then
            ...
            let countryOrCode = 
                match countryCodesDict |> Map.tryFind topCountry.CountryCode with
