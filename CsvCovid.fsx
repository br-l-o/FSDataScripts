...
let basicCovidDataByDay startFromToday =
    let orderedCovidData =
        if startFromToday then
            covidData.Rows
            |> Seq.sortByDescending (fun x -> x.Submission_date)
        else
            covidData.Rows
            |> Seq.sortByDescending (fun x -> x.Submission_date)
...
let StatesByCovidCases ascending =
    let sortedCaseRates =
        if ascending then
            caseRates.Rows
            |> Seq.sortBy (fun x -> x.``Total Cases``)
        else
            caseRates.Rows
            |> Seq.sortByDescending (fun x -> x.``Total Cases``)
...
let stateVaxxData =
    StateVaccinationData.Load(stateCovidVaxxCsv)
...
let stateVaxxDataSerialized =
    stateVaxxData.Rows
    // this column could come up null
    |> Seq.filter
        (fun x ->
            not
            <| System.Double.IsNaN(x.Additional_Doses_Vax_Pct |> double))
...
let orderStatesByCurrentTotalDoses ascending =
    let maxDateTime =
        stateVaxxDataSerialized
        |> Seq.map (fun x -> x.Date)
        |> Seq.max

    let mostCurrentSerializedData =
        stateVaxxDataSerialized
        |> Seq.filter (fun x -> x.Date = maxDateTime)

    if ascending then
        mostCurrentSerializedData
        |> Seq.sortBy (fun x -> x.TotalDoses)
    else
        mostCurrentSerializedData
        |> Seq.sortByDescending (fun x -> x.TotalDoses)


[<Literal>]
let vaxxHesitanceJson =
    "https://data.cdc.gov/api/views/q9mh-h2tw/rows.csv?accessType=DOWNLOAD"

type VaccineHesitance = CsvProvider<vaxxHesitanceJson>
let vaxxHesitanceData = VaccineHesitance.Load(vaxxHesitanceJson)

type VaccineHesitanceData =
    { State: string
      County: string
      Vaccinated: float
      Hesitant: decimal
      StronglyHesitant: decimal }

let SerializeVaccineHesitanceData (row: VaccineHesitance.Row) =
    { State = row.State
      County = row.``County Name``
      Vaccinated = row.``Percent adults fully vaccinated against COVID-19 (as of 6/10/21)``
      Hesitant = row.``Estimated hesitant or unsure``
      StronglyHesitant = row.``Estimated strongly hesitant`` }

let serializedVaxxData =
    vaxxHesitanceData.Rows
    |> Seq.filter
        (fun x ->
            not
            <| System.Double.IsNaN(x.``Social Vulnerability Index (SVI)`` |> double))
    |> Seq.map SerializeVaccineHesitanceData

let mostHesitantArea =
    serializedVaxxData
    |> Seq.maxBy (fun x -> x.Hesitant)

let leastHesitantArea =
    serializedVaxxData
    |> Seq.minBy (fun x -> x.Hesitant)
