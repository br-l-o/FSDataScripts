...
let meteoriteLandingValues =
    MeteoriteLanding.Load(meteoriteLandingsJson)

let groupedPeople =
    peopleSpaceValues.People
    |> Array.groupBy (fun x -> x.Craft)

let groupedPeopleCount =
    groupedPeople
    |> Array.map (fun x -> fst x, snd x |> Array.length)

let groupedPeopleList =
    groupedPeople
    |> Array.map (fun x -> fst x, snd x |> Array.map (fun y -> y.Name))
...
let GetYearlyMeteoriteLandingsCount year =
    year |> GetYearlyMeteoriteLandings |> Array.length
